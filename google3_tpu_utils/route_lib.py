...
# The only states in which a run has truly ENDED, so its queue row is a
# historical record safe to archive / reap. Deliberately NARROWER than
# TERMINAL_STATES, which counts RUNNING as "terminal" only in the sense of "the
# router has stopped tracking it" -- a RUNNING row is a job LIVE on the cluster,
# and archiving it would drop the record while the work continues. This is the
# ONE source of truth for "finished": `tpu clear`'s queue cascade
# (partition_for_archive below) and queue_cli's snapshot GC both key off it, so
...
def entry_xids(entry: 'QueueEntry') -> set:
  """Every XID this row has ever held -- current plus history -- as strings.

  `xid` is overwritten on each (re-)placement, so the current value alone misses
  the experiments an earlier attempt launched. `prior_xids` keeps those, and a
  cross-reference from a board XID to its queue row has to search BOTH or it
  silently fails to find the row for any job that was ever re-dispatched.

  Pure: no I/O, no mutation. Empty/None ids are dropped so an unplaced row
  (xid=None) contributes nothing rather than a bogus '' that would match another
  unplaced row.
  """
  out = set()
...
class ArchivePartition:
  """How `tpu clear`'s XID list splits the local queue. Pure data, no I/O.

  * `to_archive`  -- rows whose run has FINISHED (DONE/FAILED) and that one of
                     the requested XIDs points at: safe to move to legacy.
  * `refused`     -- rows a requested XID points at but that are STILL LIVE
                     (QUEUED/BUILDING/SUBMITTED/RUNNING/...). Never archived:
                     the row is how the router still tracks a job on the
                     cluster, and dropping it strands the work. (job_id, state,
                     xid) so the caller can warn precisely.
  * `matched_xids`-- the subset of requested XIDs that hit SOME row (archived or
                     refused). A requested XID absent here is simply board-only
                     (the common case: most runs never had a local-queue row),
                     which is not an error.
  """
  to_archive: list  # list[QueueEntry]
  refused: list     # list[tuple[str, str, str]] = (job_id, state, xid)
  matched_xids: set


def partition_for_archive(entries: Sequence['QueueEntry'],
                          xids: set) -> ArchivePartition:
  """Split the queue by a set of XIDs `tpu clear` is archiving off the board.

  Matching is by XID (current OR historical), because that is the identity the
  board and `tpu clear` speak; a queue row's own `job_id` is a different,
  local-only name. A row matches when any of its `entry_xids` is in `xids`.

  A matched row is archived ONLY if its state is in FINISHED_STATES. A matched
  row that is still live is REFUSED, never archived -- mirroring exactly why
  `tpu dequeue` refuses a live row: the queue row is not the job, and removing
  it does not stop work on the cluster, it only removes the router's handle on
  it. Keeping the two rules identical (both anchored on FINISHED_STATES /
  its complement) is deliberate: archive and dequeue must never disagree about
  which rows are safe to drop.

  Pure: no I/O, no mutation of the entries. The caller performs the actual
  move under the queue lock.
  """
  ...
  matched = set()
  for e in entries:
    hit = entry_xids(e) & want
    if not hit:
      continue
    matched |= hit
    if e.state in FINISHED_STATES:
      to_archive.append(e)
    else:
      state = getattr(e.state, 'value', e.state)
      refused.append((e.job_id, str(state), str(getattr(e, 'xid', '') or '')))
  return ArchivePartition(to_archive=to_archive, refused=refused,
                          matched_xids=matched)
...
def decide_inplace_reroute(
    xm_status: str,
    restarts_since_progress: Optional[int],
    threshold: int = 3,
    running_const: str = 'RUNNING',
) -> bool:
  """Pure: force a nominally-RUNNING row back into the auction when Borg keeps
  restarting it IN PLACE without making durable progress.

  ★THE GAP THIS CLOSES. `needs_reroute` selects SUBMITTED rows only, and
  `decide_reroute`'s `output_is_fresh` guard treats any recent write as life.
  When Borg preempts a task gang and RESTARTS IT ON THE SAME SLICE, the work
  unit never drops to PENDING and never loses its allocation, so the SUBMITTED
  path cannot see it; and its restart banners keep the output dir 'fresh', so
  the RUNNING path promotes it as alive every pass. The job therefore churns
  forever on a cell that keeps evicting it, and the smart router -- which only
  re-picks arch+metro when a row is RE-QUEUED -- never gets the chance (measured
  on xid 288745238: ~10 in-place restarts in <2h on one v5p cell, checkpoint
  frozen at the resume step the whole time, reroutes=0).

  `restarts_since_progress` is how many times the job has restarted in place
  SINCE its newest checkpoint -- i.e. with zero durable progress -- measured off
  CNS by the caller's probe. None means the probe could not measure it (no
  bucket resolvable, no attempt logs yet, lookup failed): an unmeasurable signal
  is NEVER a verdict, so we do nothing, exactly like every other unknown here.

  ★THE ZERO-PROGRESS CONDITION IS THE WHOLE POINT, and it is what makes this
  safe to run automatically. A job that IS checkpointing between preemptions
  resets the count to zero each time it saves, so it is left alone no matter how
  often the market moves it -- only a job that is BOTH thrashing AND getting
  nowhere is pulled back. That is strictly narrower than 'preempted a lot': a
  healthy run that loses 1000 steps to a preemption but re-saves is invisible
  here.

  Returns True only when the row is RUNNING AND the probe measured at least
  `threshold` no-progress restarts.
  """
  if xm_status != running_const:
    return False
  if restarts_since_progress is None:
    return False
  return restarts_since_progress >= threshold
...
def record_eviction(entry: QueueEntry, now: float) -> QueueEntry:
  ...
  Strikes on a cell accumulate (a cell that threw this job out twice sorts worse
  than one that did it once) and `evict_penalty` decays them to nothing over
  EVICT_DECAY_S, so the record is a fading hint about the market, never a
  permanent ban. MUST be called while `entry.cell` still names the offending
