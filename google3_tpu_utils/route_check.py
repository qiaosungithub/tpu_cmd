...
class _PlacementProbe(Protocol):
  """Reads a job's landing (cell, arch, chips) from XManager for a row that lost
  it locally (e.g. adopted from an escaped build). None on any failure -- a
  placement that cannot be read is never guessed."""

  def placement_of(self, xid: str) -> 'Optional[tuple[str, str, int]]':
    ...
...
class _RestartProbe(Protocol):
  ...
  CNS listing in production, scripted in tests. None NEVER means 'reroute' -- an
  unmeasurable signal is 'do nothing', the same fail-safe bias as every other
  probe here."""
...
_INPLACE_REROUTE = flags.DEFINE_bool(
    ...
    'PENDING, so the ordinary reroute path is blind to it) without making '
    'durable progress. Closes the gap where such a job churns forever on a cell '
    'that keeps evicting it while the smart router -- which only re-picks '
    'arch+metro on RE-QUEUE -- never gets a turn. Detected from CNS: in-place '
    'restarts since the newest checkpoint. Any doubt leaves the row alone.')
_INPLACE_RESTART_THRESHOLD = flags.DEFINE_integer(
    'inplace_restart_threshold', 3, 'How many in-place restarts since the last '
    'checkpoint (= restarts that produced ZERO durable progress) force a '
    'nominally-RUNNING row back into the auction for a real re-route. Operator '
    'default: 3. A job that checkpoints between preemptions resets this and is '
    'never touched, however often the market moves it.')
...
# --- process-level singleton BUILDER lock ---------------------------------
# The BUILDING flock in claim_next_build serializes one build IN FLIGHT, but it
# does NOT stop two builder PROCESSES from co-existing and alternating claims.
# That is the double-drain bug: a legacy `--worker` (serial builder) and a
# `--dispatch_worker` (router+builder) both live on ~/.tpu_local_queue.json,
# each claiming and building; worse, `--worker` claims straight from QUEUED, so
# it also bypasses the dispatch worker's budget admission. This lock guarantees
# AT MOST ONE builder process per queue file, ever. It is held for the whole
# process lifetime (the fd is dropped only when the process exits, which frees
# the flock); keying it on the queue file keeps the tpu and npu builders
# independent (each queue gets its own single builder).
_builder_singleton_fh = None  # module-global keeps the LOCKED fd alive


def _builder_lockfile(queue_file: str) -> str:
  # Distinct from `{queue_file}.lock` (the brief per-claim queue lock), because
  # this one is held for the process lifetime and must never block a claim.
  return f'{queue_file}.builder.lock'


def _try_lock_builder(queue_file: str, role: str):
  """Non-blocking attempt to grab THE builder lock for `queue_file`. Returns the
  LOCKED file handle (keep it alive to hold the lock) on success, or None if
  another builder already holds it. Factored out of acquire_builder_singleton so
  tests can hold and release the lock explicitly without touching module state.

  Opened 'a+' (create-if-missing, never truncate-on-open) so a LOSER that opens
  the file just to attempt the lock does not wipe the winner's holder stamp; the
  winner truncates+rewrites the stamp only AFTER it holds the flock."""
  import socket
  fh = open(_builder_lockfile(queue_file), 'a+')
  try:
  ...
  try:
    fh.seek(0)
    fh.truncate()
    fh.write(f'{role} pid={os.getpid()} host={socket.gethostname()} '
             f'acquired={time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}\n')
    fh.flush()
  except OSError:
    pass  # the lock is what matters; the stamp is only for diagnostics
  return fh


def acquire_builder_singleton(queue_file: str, role: str) -> bool:
  """Become THE builder for `queue_file`. Returns True if this process now holds
  the exclusive builder lock, False if another builder already does (in which
  case the caller MUST NOT build -- it should exit). Non-blocking: the loser
  returns at once rather than queueing up behind the holder."""
  global _builder_singleton_fh
  fh = _try_lock_builder(queue_file, role)
  if fh is None:
    return False
  _builder_singleton_fh = fh  # keep the fd alive -> lock held until exit
  return True


def read_builder_singleton_holder(queue_file: str) -> str:
  """Best-effort description of who currently holds the builder lock, for a log
  line. Reading is safe while another process holds the flock (flock does not
  block reads); an empty/missing file yields 'unknown'."""
  try:
    with open(_builder_lockfile(queue_file)) as fh:
      s = fh.read().strip()
    return s or 'unknown'
  except OSError:
    return 'unknown'
...
def merge_and_save_touched(
  ...
  Pass baseline={e.job_id: e.to_dict() for e in entries} BEFORE running slow
  work. Pass functions return their entire input, including unchanged rows.
  With a baseline, unchanged rows are ignored, and changed rows are committed
  only if the live row still equals its baseline. Conflicts are left for the
  next pass to re-evaluate; a stale pass must never undo a worker's submission.
  Rows deleted concurrently are not resurrected. Callers without a baseline
  retain the legacy API for an explicitly selected set of touched rows.

  Ordering: preserve live order and append only genuinely new rows.
  """
  ...
  with with_queue_lock(path):
    ...
    seen = set()
    for e in live:
      if baseline is not None and e.to_dict() != baseline.get(e.job_id):
        # Another writer changed/created this row while the RPCs were running.
        merged.append(e)
        seen.add(e.job_id)
        continue
      ...
      if e.job_id in touched_by_id:
        ...
        if baseline is not None and t.to_dict() == baseline.get(e.job_id):
          merged.append(e)
          seen.add(e.job_id)
          continue
    ...
    for e in touched:
      if (e.job_id not in seen and e.job_id not in dropped
          and (baseline is None or e.job_id not in baseline)):
        merged.append(e)
    save_queue(path, merged)


def archive_finished_queue_rows(path: str, xids: set) -> dict:
  """Remove the FINISHED local-queue rows that a set of board XIDs points at,
  and return them so the caller can fold them into the archive record.

  This is `tpu clear`'s queue half. `tpu clear` archives board entries by XID;
  this drops the matching queue rows so a cleared run does not linger in
  `tpu queue-status`. It NEVER touches a live row: route_lib.partition_for_archive
  archives only DONE/FAILED rows and refuses the rest, exactly as `tpu dequeue`
  refuses a live row -- the queue row is the router's handle on a job still on
  the cluster, and dropping it strands the work.

  The whole read-modify-write is under the queue lock, like every other queue
  mutation, so a concurrent route tick or enqueue cannot clobber (or be
  clobbered by) the removal. Snapshots of the removed rows are reaped too --
  the row is gone, so nothing will ever build from them again.

  Returns a dict with:
    'archived': list[QueueEntry]           rows removed from the queue
    'refused':  list[(job_id, state, xid)] matched rows still LIVE, left in place
    'matched_xids': set[str]               requested XIDs that hit some row
  A requested XID that matched no row at all is simply board-only and is not
  reported here (the board half of _clear_jobs handles it).
  """
  want = {str(x) for x in xids if x}
  if not want:
    return {'archived': [], 'refused': [], 'matched_xids': set()}
  with with_queue_lock(path):
    entries = load_queue(path)
    part = route_lib.partition_for_archive(entries, want)
    if part.to_archive:
      drop_ids = {e.job_id for e in part.to_archive}
      keep = [e for e in entries if e.job_id not in drop_ids]
      save_queue(path, keep)
  # The archived rows' enqueue snapshots are left for the enqueue-time orphan GC
  # to reap: `_gc_orphan_snapshots` removes any snapshot dir whose job_id is no
  # longer in the queue (after a short grace window), which these rows now
  # satisfy. Not calling queue_cli here is deliberate -- route_check must not
  # import queue_cli (queue_cli imports route_check, so it would be a cycle), and
  # a ~15 MB dir reaped on the next enqueue is not worth breaking that layering.
  return {'archived': part.to_archive, 'refused': part.refused,
          'matched_xids': part.matched_xids}
...
def parse_placement_from_launch_args(
    launch_args: 'Optional[Sequence[object]]',
) -> Optional[tuple[str, str, int]]:
  """(cell, arch, chips) from an experiment's recorded launch_args, or None.

  The queue launcher records the RESOLVED placement verbatim on the experiment
  as `--tpu_type=<arch>-<chips>` and `--cell=<cell>` (see build_tpu_queue_cmd
  and xm_launcher). Both live on the Experiment proto and survive the job's
  death, so this recovers the placement of a row whose local cell/arch/chips
  were lost -- specifically a row adopted from an escaped build, which sets the
  xid but never runs apply_placement (the only writer of cell/arch/chips). That
  gap is what left xid 288485310 with cell=None, blinding both liveness probes.

  Pure and offline: the caller does the XManager read and passes launch_args in.
  """
  cell = None
  tpu_type = None
  for a in (launch_args or []):
    s = str(a)
    if s.startswith('--cell='):
      cell = s.split('=', 1)[1].strip() or None
    elif s.startswith('--tpu_type='):
      tpu_type = s.split('=', 1)[1].strip() or None
  if not cell or not tpu_type:
    return None
  # tpu_type is '<arch>-<chips>'. rpartition on the LAST '-' so an arch token
  # that itself contains no '-' (b200, h100, v5p) splits cleanly, and a future
  # 'a100_80gib-8' would too.
  arch, _, chips_s = tpu_type.rpartition('-')
  if not arch or not chips_s.isdigit():
    return None
  return cell, arch.lower(), int(chips_s)


class XManagerPlacementProbe:
  """Reads a job's landing placement (cell, arch, chips) from XManager, for a
  row that lost it locally. Returns None on any failure -- a placement we cannot
  read is never guessed (the same fail-safe bias as XManagerStatusProbe)."""
  ...
  def placement_of(self, xid: str) -> Optional[tuple[str, str, int]]:
    try:
      exp = self._get_client().get_experiment(int(xid))
...
class CnsRestartProbe:
  """How many times Borg has restarted a job IN PLACE since it last made durable
  progress -- the signal `route_lib.decide_inplace_reroute` acts on.

  ★WHY THIS EXISTS. A preempted task gang that Borg restarts ON THE SAME SLICE
  never drops to PENDING and never loses its allocation, so the SUBMITTED
  re-route path (needs_reroute selects SUBMITTED only) cannot see it; and each
  restart writes a fresh boot banner, so the output-fresh guard reads it as
  alive every pass. Such a job churns forever on a cell that keeps evicting it,
  and the smart router -- which only re-picks arch+metro when a row is
  RE-QUEUED -- never gets a turn (measured on xid 288745238).

  The measurement is entirely off CNS, no service in the loop:
    * every in-place restart writes one more `logs/rank_0_attempt<N>.log`;
    * every durable step-of-progress writes one more `checkpoints/<step>/` leaf
      (or, torch, `steps/step_<N>.pt`).
  So the count of rank-0 attempt logs NEWER than the newest COMPLETE checkpoint
  is exactly 'restarts that produced no progress'. A job that checkpoints
  between preemptions drives this back toward 1 on every save and is never
  flagged; only one that is BOTH restarting AND getting nowhere climbs past the
  threshold.

  Any failure -- no bucket/XID, fileutil error/timeout, no attempt logs at all --
  returns None ('cannot measure'), which the pure decision reads as 'do
  nothing'. A broken read can only make the detector MORE cautious, never fire
  it wrongly. An incomplete orbax checkpoint (`<N>.orbax-checkpoint-tmp-<uuid>`)
  has a non-bare-int leaf and is skipped, so a half-written save never reads as
  progress.
  """
  ...
  def _newest_checkpoint_mtime(self, bucket: str, xid: str) -> Optional[float]:
    ...
    `fileutil ls -l <base>/checkpoints` lists the leaf dirs (<step>/) with THEIR
    own mtimes -- the finalize time of each checkpoint -- so no -R/-d is needed.
    A bare-int leaf is complete by construction (orbax renames the tmp dir on
    finalize); torch's `steps/step_<N>.pt` is the file-based equivalent.
    """
    base = f'{bucket.rstrip("/")}/logs/*/xid_{xid}_*'
    newest: Optional[float] = None
    for parent in ('checkpoints', 'steps'):
      rows = self._ls_l(f'{base}/{parent}')
      if not rows:
        continue
      for fields in rows:
        if len(fields) < 8 or fields[0].startswith('total'):
          continue
        leaf = fields[-1].rstrip('/').rsplit('/', 1)[-1]
        complete = leaf.isdigit() or (leaf.startswith('step_')
                                      and leaf.endswith('.pt'))
        if not complete:
          continue
        mt = _parse_fileutil_mtime(fields)
...
def run_reroute(
  ...
  for e in candidates + [r for r in recheck if r not in candidates]:
    ...
    if state == STATUS_PENDING and xid:
      ...
      # ★RECOVER A LOST CELL FIRST. A RUNNING row with no cell is the
      # adopt-escaped-build defect: the xid was set without apply_placement, so
      # cell/arch/chips stayed None. Both liveness probes below need the cell --
      # has_running_vmgroup builds `borg findjobs` on it, _bucket_for_entry
      # resolves the output path from it -- so a cell-less row blinds BOTH, and
      # the old code fell straight through to "promoted (borg unreadable)"
      # forever (xid 288485310 sat 16h that way). XManager still records the
      # resolved placement in launch_args, and the same XM API just answered
      # STATUS_RUNNING so it is reachable; recover the cell here so this pass --
      # and every future one, and the persisted row -- can actually see the job.
      if not (e.cell or '').strip() and xid and placement_probe is not None:
        pl = placement_probe.placement_of(xid)
        if pl is not None:
          e.cell, e.arch, e.chips = pl
          tag = f'{e.job_id} (xid={xid}, {e.cell}, pending {age}s)'
          log.append(f'[reroute] {tag} was RUNNING with no cell -> recovered '
                     f'{e.arch}-{e.chips}@{e.cell} from XM launch_args')
      # ★IN-PLACE PREEMPTION THRASH -- the gap the SUBMITTED and output-fresh
      # guards structurally cannot see (route_lib.decide_inplace_reroute owns
      # the pure decision). A gang Borg keeps restarting on the SAME slice never
      # goes PENDING and keeps its output dir "fresh", so the branch below would
      # promote it as alive every pass while it churns and the router never
      # re-picks its cell. Count restarts since the newest checkpoint = restarts
      # that made ZERO durable progress; a job that checkpoints between
      # preemptions resets the count and is left alone no matter how often it is
      ...
      if vm_running is False and age >= nominal_running_grace_s and xid:
        ...
        # "unqueryable" -- there is no cell to form the query -- not the
        # transient "the RPC failed" that the fail-open below was written to
        # tolerate, and it will NEVER become queryable on its own. The output
        # probe is equally blind (no bucket without a cell). So XM's RUNNING is
        # the only signal left, and it is precisely the one this branch exists
        # because it lies. A correctly placed job always carries a cell, so past
        # grace a cell-less RUNNING row is the wedge, not a live run -- treat it
        # as stuck rather than promoting it beyond reach forever.
        nominal = True
      # `or not xid` is redundant against the guards above (nominal can only be
...
def run_worker_once(
  ...
  if is_budget_deferral(out):
    def _deferred(e: route_lib.QueueEntry) -> None:
      route_lib.mark_budget_deferred(
          e, f'budget-deferred at build: over the G9 credit bar; will '
          f'auto-retry when headroom opens (attempts untouched at '
          f'{e.attempts})')
  ...
  if attempts_after >= max_build_attempts:
    reason = (f'build failed {attempts_after} times, never produced an XID '
              f'(build crashed or left a found[] zombie work unit). '
              f'Fix the build, then: tpu requeue {claimed.job_id}')
  ...
  def _failed(e: route_lib.QueueEntry) -> None:
    ...
    e.worker_id = None
    e.attempts = attempts_after
    e.last_reason = (f'build produced no XID (crash or found[] zombie); '
                     f'auto-retry {attempts_after}/{max_build_attempts}')
...
def main(argv):
  ...
  if _WORKER.value:
    ...
    # SINGLETON GUARD: at most one builder process per queue file, ever. The
    # BUILDING flock only serializes one build in flight; this stops a SECOND
    # builder process (e.g. a legacy --worker next to a --dispatch_worker) from
    # co-existing on the same queue and double-draining it. Loser exits cleanly.
    if not acquire_builder_singleton(_QUEUE_FILE.value, role='worker'):
      print(f'[worker] another builder already owns {_QUEUE_FILE.value} '
            f'({read_builder_singleton_holder(_QUEUE_FILE.value)}); exiting so '
            f'only one builder runs.', flush=True)
      return
  ...
  if _DISPATCH_WORKER.value:
    ...
    import socket
    worker_id = f'{socket.gethostname()}:{os.getpid()}'
    # SINGLETON GUARD: see the --worker branch. Same builder lock, so a
    # dispatch-worker and a legacy worker can never both drain one queue.
    if not acquire_builder_singleton(_QUEUE_FILE.value, role='dispatch_worker'):
      print(f'[dispatch-worker] another builder already owns '
            f'{_QUEUE_FILE.value} '
            f'({read_builder_singleton_holder(_QUEUE_FILE.value)}); exiting so '
            f'only one builder runs.', flush=True)
      return
    ...
    while True:
      ...
      try:
        ...
        if not _DRY_RUN.value:
          merge_and_save_touched(_QUEUE_FILE.value, rc_entries, baseline=baseline)
      except Exception as e:  # pylint: disable=broad-except
      ...
      try:
        ...
        if not _DRY_RUN.value:
          merge_and_save_touched(_QUEUE_FILE.value, rr_entries, baseline=baseline)
      except Exception as e:  # pylint: disable=broad-except
  ...
  baseline = {e.job_id: e.to_dict() for e in entries}
  ...
  if not _DRY_RUN.value:
    ...
    merge_and_save_touched(_QUEUE_FILE.value, updated, baseline=baseline)
